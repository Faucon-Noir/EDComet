...
| Now | Deliver SSE journal updates MVP (metadata-only events + targeted re-fetch) | 9 | 8 | 5 | 360 |
...
- SSE MVP decision: emit metadata-only journal updates, then re-fetch the affected REST resource from the frontend.
- Supported MVP event families: `Loadout`, `ColonisationConstructionDepot`, `FileHeader`.
